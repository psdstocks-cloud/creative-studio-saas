...
export const onRequest = async (
  ...
  try {
    ...
    // Build path: /stockorder/{site}/{id}
    const path = `/stockorder/${encodeURIComponent(site)}/${encodeURIComponent(id)}`;
    ...
    // Add user info for audit trail (if upstream supports it)
    if (user) {
    ...
    if (!response.ok) {
      ...
      return errorResponse(
        ...
        `Failed to place stock order: ${errorBody || response.statusText}`
      );
    }
    ...
    const message = error?.message || 'Failed to place stock order';
    const status = /access token/i.test(message) ? 401 : 502;
    console.error('Stockorder proxy error:', error);
