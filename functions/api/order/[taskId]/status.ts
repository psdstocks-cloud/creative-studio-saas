...
export const onRequest = async (
  ...
  try {
    ...
    // Build path: /order/{taskId}/status
    const path = `/order/${encodeURIComponent(taskId)}/status`;
    ...
    if (!response.ok) {
      ...
      return errorResponse(
        ...
        `Failed to check order status: ${errorBody || response.statusText}`
      );
    }
    ...
    const message = error?.message || 'Failed to check order status';
