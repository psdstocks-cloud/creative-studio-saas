...
const SESSION_PERSIST_PATH =
  process.env.SESSION_PERSIST_PATH || path.join(__dirname, 'logs', 'sessions.json');
const AUDIT_WEBHOOK_URL = process.env.AUDIT_WEBHOOK_URL || null;
const AUDIT_WEBHOOK_AUTH_HEADER = process.env.AUDIT_WEBHOOK_AUTH_HEADER || null;
...
const REDIS_URL = process.env.SESSION_REDIS_URL || process.env.REDIS_URL || null;
const REDIS_TLS = String(process.env.SESSION_REDIS_TLS || process.env.REDIS_TLS || '').toLowerCase() === 'true';
const SUPABASE_URL =
...
const OBSERVABILITY_LOG_PATH =
  process.env.OBSERVABILITY_LOG_PATH || path.join(__dirname, 'logs', 'observability.log');
const OBSERVABILITY_WEBHOOK_URL = process.env.OBSERVABILITY_WEBHOOK_URL || null;
const OBSERVABILITY_WEBHOOK_AUTH_HEADER = process.env.OBSERVABILITY_WEBHOOK_AUTH_HEADER || null;
...
if (REDIS_URL) {
  try {
    ...
    console.warn(
      'ℹ️  Redis client library is not available. Falling back to persistent file-based sessions.',
      error?.code === 'ERR_MODULE_NOT_FOUND' ? '' : error
    );
  }
}
...
if (REDIS_URL && Redis) {
  try {
    ...
    redisClient.on('connect', () => {
      console.info('✅ Connected to Redis session store');
    });
  } catch (error) {
    console.error('Failed to initialize Redis client. Falling back to persistent sessions.', error);
    redisClient = null;
  }
} else if (REDIS_URL && !Redis) {
  console.warn('ℹ️  REDIS_URL provided but redis client library is unavailable. Using file-based sessions.');
}
...
class MemorySessionStore {
  ...
  get cleanupIntervalMs() {
    return Math.max(SESSION_TTL_MS / 2, 60_000);
  }
}

class FileSessionStore extends MemorySessionStore {
  ...
  async loadFromDisk() {
    try {
      ...
      if (error && error.code !== 'ENOENT') {
        console.error('Failed to hydrate sessions from disk', error);
      }
    }
  }

  schedulePersist() {
    if (this.isPersistScheduled) {
      return;
    }
    ...
    setTimeout(() => {
      ...
      this.persist().catch((error) => {
        console.error('Failed to persist sessions to disk', error);
      });
    }, 25).unref();
  }

  async persist() {
    const payload = JSON.stringify(Array.from(this.sessions.values()));
    await fs.promises.writeFile(this.filePath, payload, 'utf8');
  }

  async set(session) {
    await super.set(session);
...
class RedisSessionStore {
  constructor(client) {
    this.client = client;
    this.prefix = 'css:sessions:';
  }

  async get(sessionId) {
    try {
      const payload = await this.client.get(`${this.prefix}${sessionId}`);
      if (!payload) {
        return null;
      }
  ...
  async cleanup() {
    // Redis manages TTL expiration automatically.
  }
...
const sessionStore = redisClient
  ...
  : new FileSessionStore(SESSION_PERSIST_PATH);

const cleanupInterval = sessionStore.cleanupIntervalMs;

if (cleanupInterval) {
  setInterval(() => {
    Promise.resolve(sessionStore.cleanup?.()).catch((error) => {
...
const sessionPersistDir = path.dirname(SESSION_PERSIST_PATH);
ensureDirectory(sessionPersistDir);

const observabilityLogDir = path.dirname(OBSERVABILITY_LOG_PATH);
ensureDirectory(observabilityLogDir);

const observabilityStream = fs.createWriteStream(OBSERVABILITY_LOG_PATH, { flags: 'a' });

const deliverAuditWebhook = async (payload) => {
  ...
  try {
    await fetch(AUDIT_WEBHOOK_URL, {
      ...
      headers: {
        ...
        ...(AUDIT_WEBHOOK_AUTH_HEADER ? { Authorization: AUDIT_WEBHOOK_AUTH_HEADER } : {}),
      },
      body: JSON.stringify(payload),
    });
  } catch (error) {
    console.error('Failed to deliver audit webhook entry', error);
  }
};
...
const writeAuditEntry = (entry) => {
  ...
  deliverAuditWebhook(payload);
};

const deliverObservabilityWebhook = async (payload) => {
    ...
    await fetch(OBSERVABILITY_WEBHOOK_URL, {
      ...
      headers: {
        ...
        ...(OBSERVABILITY_WEBHOOK_AUTH_HEADER
          ? { Authorization: OBSERVABILITY_WEBHOOK_AUTH_HEADER }
          : {}),
      },
      body: JSON.stringify(payload),
    });
  } catch (error) {
    console.error('Failed to deliver observability webhook entry', error);
  }
};

const recordObservabilityEvent = (event) => {
  const payload = {
    ...event,
    timestamp: new Date().toISOString(),
  };
  ...
  try {
    observabilityStream.write(`${JSON.stringify(payload)}\n`);
  } catch (error) {
    console.error('Failed to write observability event', error);
  }

  deliverObservabilityWebhook(payload);
};
  ...
  res.on('finish', () => {
    ...
    if (res.statusCode >= 500) {
      recordObservabilityEvent({
        level: 'error',
        message: `HTTP ${res.statusCode} ${method} ${originalUrl}`,
    ...
    if (sessionId) {
      ...
      if (session) {
        if (session.expiresAt <= Date.now()) {
          ...
          const shouldRefresh = await refreshSession(session);
          if (shouldRefresh) {
            res.append(
              'Set-Cookie',
              buildSessionCookie(session.id, { expiresAt: session.expiresAt })
            );
          }
        }
      }
...
app.delete('/api/auth/session', requireAuth, async (req, res) => {
  if (req.session) {
...
app.post('/api/observability/events', (req, res) => {
  const { level = 'info', message, stack, componentStack, context } = req.body || {};
  ...
  recordObservabilityEvent({
    level: allowedLevels.has(normalizedLevel) ? normalizedLevel : 'info',
    message,
...
if (process.env.NODE_ENV !== 'test') {
  app.listen(port, () => {
    ...
    console.log(`Observability log located at: ${OBSERVABILITY_LOG_PATH}`);
  });
}

export { buildSessionCookie, parseCookies, MemorySessionStore, FileSessionStore };
