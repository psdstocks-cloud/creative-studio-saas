...
class QueryClient {
  ...
  async ensureQuery<TData>(queryKey: QueryKey, queryFn: () => Promise<TData>, options: EnsureOptions = {}) {
  ...
  setQueryData<TData>(queryKey: QueryKey, updater: QueryUpdater<TData>) {
    ...
    const next = typeof updater === 'function' ? (updater as (value: TData | undefined) => TData)(previous) : updater;
    entry.data = next;
    entry.error = undefined;
    entry.status = 'success';
    entry.lastUpdated = Date.now();
