...
export const fetchAccountOverview = async (): Promise<AccountOverview> => {
  ...
  const plan = firstString(
    ...
    (raw.subscription as Record<string, unknown> | undefined)?.plan &&
      ...
      ? firstString(
          ((raw.subscription as Record<string, unknown> | undefined)?.plan as Record<string, unknown>).name,
          ((raw.subscription as Record<string, unknown> | undefined)?.plan as Record<string, unknown>).id,
        )
      : null,
  );

  return {
    id: firstString(raw.id, (raw.user as Record<string, unknown> | undefined)?.id, metadata?.id) ?? null,
    email: firstString(raw.email, (raw.user as Record<string, unknown> | undefined)?.email, metadata?.email) ?? null,
    username: firstString(raw.username, (raw.user as Record<string, unknown> | undefined)?.username, metadata?.username) ?? null,
    balance,
    plan: plan ?? null,
    lastLoginAt:
      firstString(
...
export const sendPoints = async ({ receiver, amount, note }: SendPointsPayload): Promise<SendPointsResult> => {
  ...
  const normalizedSuccess = typeof successFlag === 'boolean'
    ? successFlag
  ...
  const balance = firstNumber(raw.balance, raw.points, raw.wallet, (raw.data as Record<string, unknown> | undefined)?.balance);
