    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-slate-400">
            Monitor live order throughput, fulfillment queues, and spend while reviewing the most recent audit activity across the platform.
          </p>
        </div>
        <button
          type="button"
      ...
      <section className="grid gap-6 lg:grid-cols-5">
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6 lg:col-span-3">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">Breakdown of where orders originated during the last 24 hours.</p>
            </div>
          </div>
          <ul className="mt-4 space-y-3">
            ...
            {!isLoading && (data?.topSites?.length ?? 0) === 0 ? (
              <li className="rounded-lg border border-dashed border-slate-700 bg-slate-950/30 px-4 py-6 text-center text-sm text-slate-500">
                No orders recorded in the last 24 hours.
              </li>
            ) : null}
          </ul>
        </div>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6 lg:col-span-2">
          ...
          <p className="mt-1 text-sm text-slate-400">Recent write operations and high-sensitivity reads captured by the BFF.</p>
          <div className="mt-4 space-y-4">
            ...
            {!isLoading && (data?.recentAudit?.length ?? 0) === 0 ? (
              <div className="rounded-lg border border-dashed border-slate-700 bg-slate-950/30 p-4 text-center text-sm text-slate-500">
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">Snapshot of the 10 most recent orders across all users.</p>
          </div>
        </div>
                  ...
                  <td className="px-4 py-3 text-slate-200">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-500">{order.file_info?.site?.toUpperCase()}</p>
                      </div>
                    </div>
                  </td>
