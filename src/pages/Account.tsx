...
const AccountPage: React.FC = () => {
  ...
  const [balanceMessage, setBalanceMessage] = useState<{ type: 'success' | 'error'; message: string } | null>(null);
  const [transferMessage, setTransferMessage] = useState<{ type: 'success' | 'error'; message: string } | null>(null);
  const [transfers, setTransfers] = useState<TransferRecord[]>([]);

  const accountError = status === 'error'
    ? error instanceof Error
      ? error.message
      : t('balanceRefreshError')
    : null;
    ...
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        setTransfers((previous) => [
          createTransferRecord(trimmedReceiver, amountValue, trimmedNote, 'success', result.message || t('sendPointsSuccess')),
          ...previous,
        ].slice(0, 5));
    ...
    <div className="space-y-8">
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border border-slate-700/60 bg-slate-900/70 p-6 shadow-xl shadow-slate-900/30">
          ...
          {accountError && (
            <p className="mt-4 text-sm text-rose-400">{accountError}</p>
          )}
        </section>
      ...
      <section className="rounded-2xl border border-slate-700/60 bg-slate-900/70 p-6 shadow-xl shadow-slate-900/30">
        ...
        {transferMessage && (
          <p className={`mt-4 text-sm ${transferMessage.type === 'success' ? 'text-emerald-400' : 'text-rose-400'}`}>
            ...
            {transfers.map((transfer) => (
              ...
              >
                <div className="flex items-center justify-between text-sm text-slate-300">
                  ...
                  <span className={transfer.status === 'success' ? 'text-emerald-400' : 'text-rose-400'}>
                    {transfer.status === 'success' ? t('transferStatusSuccess') : t('transferStatusFailed')}
                  </span>
                </div>
                ...
                {transfer.note && (
                  <p className="mt-2 text-xs text-slate-500">{transfer.note}</p>
                )}
