...
import { SparklesIcon, ArrowPathIcon, CpuChipIcon, XCircleIcon, ExclamationTriangleIcon } from './icons/Icons';
          ...
          <div className="animate-fadeIn mt-6">
            ...
            {(job.status === 'pending' || job.status === 'processing') && (
              <div>
                ...
                <p className="text-center text-sm text-gray-500 dark:text-gray-400">{job.percentage_complete || 0}{t('percentComplete')}</p>
              </div>
            )}
            ...
            {job.status === 'failed' && (
                <div className="text-center p-6 bg-red-900/30 rounded-lg">
                    ...
                    <p className="text-sm text-red-400 mt-1">{job.error_message || "An unknown error occurred."}</p>
                </div>
            )}
