...
const FilesManager = () => {
  ...
  const StatusIndicator = ({ status }: { status: Order['status'] }) => {
    ...
    return (
      <div
        className={`flex items-center space-x-2 rtl:space-x-reverse text-sm font-semibold ${current.color}`}
      >
        {current.icon}
    ...
    if (orders.length === 0) {
      return (
        <div className="text-center py-16">
          ...
          <Link
            to="/stock"
            className="mt-4 inline-block bg-blue-600 text-white font-bold py-2 px-5 rounded-lg hover:bg-blue-700 transition-colors"
          >
            {t('downloadStockFile')}
          </Link>
        </div>
        ...
        <table className="min-w-full divide-y divide-gray-700">
          <thead className="bg-gray-800">
            <tr>
              <th
                scope="col"
                className="px-6 py-3 text-start text-xs font-medium text-gray-300 uppercase tracking-wider"
              >
                {t('file')}
              </th>
              <th
              ...
              <th
                scope="col"
                className="px-6 py-3 text-end text-xs font-medium text-gray-300 uppercase tracking-wider"
              >
                {t('action')}
              </th>
            </tr>
          ...
          <tbody className="bg-gray-800/50 divide-y divide-gray-700">
            {filteredOrders.map((order) => {
              ...
              const stockUrl =
                order.file_info.source_url ||
                buildStockMediaUrl(order.file_info.site, order.file_info.id);
              ...
              return (
                <tr key={order.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                    {order.file_info.cost?.toFixed(2)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-xs">
                    {order.file_info.debugid ? (
                      <div className="flex items-center gap-2 group">
                        <span className="font-mono text-gray-400">{order.file_info.debugid}</span>
                        <button
                          onClick={() => {
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-end">
                    {order.status === 'ready' && (
                      ...
                      >
                        {downloading.has(order.task_id) ? (
                          <ArrowPathIcon className="w-4 h-4 animate-spin me-2" />
                        ) : (
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="bg-gray-800/80 rounded-xl shadow-lg p-1 glassmorphism">{renderContent()}</div>
    </div>
  );
};
