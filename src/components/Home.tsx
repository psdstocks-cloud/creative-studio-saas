...
const FeatureCard = ({ title, description, icon, onClick }: FeatureCardProps) => (
  <button
    onClick={onClick}
    className="p-6 text-left w-full transition-all duration-300 rounded-2xl glassmorphism hover:bg-white/20 dark:hover:bg-white/10 hover:scale-105 border border-transparent hover:border-white/30 dark:hover:border-white/20"
  >
  ...
  return (
    <div className="animate-fadeIn space-y-12 h-full flex flex-col items-center justify-center">
      <div className="text-center">
        <h1 className="text-4xl lg:text-5xl font-bold text-gray-900 dark:text-white mb-2">{t('homeTitle')}</h1>
        <p className="text-lg text-gray-700 dark:text-gray-300 max-w-2xl mx-auto">
          Welcome back, <span className="font-semibold text-blue-600 dark:text-blue-400">{user?.email}</span>!
        </p>
        <p className="text-base text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mt-2">{t('homeSubtitle')}</p>
      </div>

      <section className="w-full max-w-5xl">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-6 text-center">{t('quickActions')}</h2>
